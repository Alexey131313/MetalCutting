...
class MaxRectsAlgorithm :  public ICuttingAlgorithm
{
public:
    CuttingResult calculate(const CuttingRequest& request) override;

private:
    std::optional<Placement> findBestPosition(const ExpandedPart& part);
    void splitFreeRectangles(const Placement& placement);
    ...
    bool contains(const Rectangle& a, const Rectangle& b) const;

private:
    std::vector<FreeRectangle> freeRectangles_;
};